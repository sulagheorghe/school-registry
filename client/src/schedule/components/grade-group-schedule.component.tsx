...
export class GradeGroupSchedule extends React.Component<
  any,
  GradeGroupScheduleState
> {
  ...
  render = () => {
    ...
    return (
      <React.Fragment>
        ...
        <DailyConfigureModal
          isOpen={showScheduleConfigureModal}
          toggle={this.toggle}
          dayOfWeek={dayOfWeek}
          updateCurrentSchedule={this.updateScheduleList}
          currentDaySchedule={activeSchedule}
  ...
  updateScheduleList = (newValues: any) => {
    console.log(this.state.gradeWeekSchedule[newValues.dayOfWeek]);
    this.setState({});
  };
    ...
    for (const weekDay in gradeWeekSchedule) {
      ...
      rows.push(
        <tr>
          ...
          <td>{schedule}</td>
          <td>
            <Button color="secondary" value={weekDay} onClick={this.showModal}>
              Configure
            </Button>
          </td>
        </tr>
