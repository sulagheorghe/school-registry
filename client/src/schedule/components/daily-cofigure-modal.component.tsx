...
import { Modal, ModalHeader, ModalBody, ModalFooter, Button, Card, CardHeader, Collapse } from 'reactstrap';
...
export class DailyConfigureModal extends React.Component<DailyConfigureModalProps, DailyConfigureModalState> {
    ...
    render = () => {
        ...
        const { showAddRecordForm, currentDaySchedule, dayOfWeek } = this.state;
        return (
            <Modal isOpen={this.props.isOpen} toggle={this.props.toggle}>
                <ModalHeader toggle={this.props.toggle}>{dayOfWeek}</ModalHeader>
                <ModalBody>
                    <ol type="1">
    ...
    submitAddRecordForm = (formData: any) => {
        const request = {...{gradeGroup:this.props.gradeGroup.id, dayOfWeek: this.props.dayOfWeek}, ...formData}
        return ApiService.post(apiRoutes.schedule,request);
    }
}